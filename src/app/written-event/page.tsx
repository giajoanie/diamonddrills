...
export default async function StudentWrittenEventPage() {
    ...
    return (
      <>
        ...
        <main className="mx-auto max-w-3xl flex-1 px-4 py-8 sm:px-6">
          <Card>
            <p className="text-foreground-muted">You&apos;re not enrolled in a written event.</p>
          </Card>
        </main>
      </>
    );
  ...
  const formattingNotes =
    checklist?.formattingRequirements && typeof checklist.formattingRequirements === "object"
  ...
  return (
    <>
      <AppHeader user={user} homeHref="/dashboard" />
      <main className="mx-auto max-w-3xl flex-1 px-4 py-8 sm:px-6">
        <h1 className="text-2xl font-semibold text-foreground">{written.event.name}</h1>
        <p className="mt-1 text-foreground-muted">{written.event.cluster.name}</p>

        <Card className="mt-6">
          <p className="mb-2 font-medium text-foreground">Checklist</p>
          {checklist?.pageLimit && (
            <p className="text-sm text-foreground-muted">Page limit: {checklist.pageLimit}</p>
          )}
          {requiredSections.length > 0 && (
            <div className="mt-2">
              <p className="text-sm font-medium text-foreground-muted">Required sections</p>
          ...
          {formattingNotes && (
            <div className="mt-2">
              <p className="text-sm font-medium text-foreground-muted">Formatting requirements</p>
              <p className="whitespace-pre-wrap text-sm text-foreground">{formattingNotes}</p>
            </div>
          )}
          {!checklist?.pageLimit && requiredSections.length === 0 && !formattingNotes && (
            <p className="text-sm text-foreground-muted">Your mentor hasn&apos;t set a checklist yet.</p>
          )}
        </Card>
            ...
            <ul className="space-y-1">
              {milestones.map((m) => (
                <li key={m.id} className="flex items-center justify-between gap-3 text-sm">
                  ...
                  <span className={m.dueAt < now ? "text-danger" : "text-foreground-muted"}>
