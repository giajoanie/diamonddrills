...
function ChangeStat({ value }: { value: number }) {
  const sign = value > 0 ? "+" : "";
  return (
    <span className={value > 0 ? "text-success" : value < 0 ? "text-danger" : "text-foreground-muted"}>
      {sign}
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-4xl flex-1 px-4 py-8 sm:px-6">
        <h1 className="text-2xl font-semibold text-foreground">PM CDE Impact Dashboard</h1>
        <p className="mt-1 text-foreground-muted">
          Before/after growth, engagement vs. improvement, and competition outcomes — formatted for
          pulling straight into a Project Management CDE written report.
        </p>
        ...
        <Card className="mt-6">
          <h2 className="mb-3 font-medium text-foreground">Before / after: score vs. baseline</h2>
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div>
              <p className="text-xs font-medium uppercase tracking-wide text-foreground-subtle">Change</p>
          ...
          <p className="mt-2 text-xs text-foreground-subtle">
            ...
            {data.scoreVsBaseline.studentCount === 1 ? "" : "s"} with both a baseline and a later exam.
          </p>
            ...
            <table className="mt-4 w-full text-sm">
              ...
              <tbody>
                {data.scoreVsBaselineByCluster.map((c) => (
                  <tr key={c.clusterId} className="border-b border-border last:border-0">
                    <td className="py-1.5 text-foreground-muted">{c.clusterName}</td>
                    <td className="py-1.5 text-foreground-muted">{Math.round(c.avgBaseline)}%</td>
                    <td className="py-1.5 text-foreground-muted">{Math.round(c.avgLatest)}%</td>
                    <td className="py-1.5">
        ...
        <Card className="mt-4">
          <h2 className="mb-1 font-medium text-foreground">Engagement vs. improvement</h2>
          <p className="mb-3 text-xs text-foreground-subtle">
            Average score change vs. baseline, grouped by how much a student has practiced on the
            platform in the selected date range.
          </p>
              ...
              <tbody>
                {data.engagementVsImprovement.map((row) => (
                  <tr key={row.bucket} className="border-b border-border last:border-0">
                    <td className="py-1.5 text-foreground-muted">{BUCKET_LABELS[row.bucket]}</td>
                    <td className="py-1.5 text-foreground-muted">{row.studentCount}</td>
                    <td className="py-1.5">
        ...
        <Card className="mt-4">
          <h2 className="mb-1 font-medium text-foreground">Competition outcomes vs. engagement</h2>
          <p className="mb-3 text-xs text-foreground-subtle">
            Advancement rate among students with a recorded competition result, grouped by engagement.
          </p>
          {data.outcomesByEngagement.length === 0 ? (
            <p className="text-sm text-foreground-muted">No competition results recorded yet.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-left text-xs uppercase tracking-wide text-foreground-subtle">
              ...
              <tbody>
                {data.outcomesByEngagement.map((row) => (
                  <tr key={row.bucket} className="border-b border-border last:border-0">
                    <td className="py-1.5 text-foreground-muted">{BUCKET_LABELS[row.bucket]}</td>
                    <td className="py-1.5 text-foreground-muted">{row.studentCount}</td>
                    <td className="py-1.5 text-foreground-muted">{Math.round(row.advancementRate)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
