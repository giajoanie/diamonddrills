...
export default async function MentorStudentProfilePage({
  ...
  const areaBreakdown = computeAreaBreakdown(
    questionHistory.flatMap((a) => a.questions),
  );

  const percentagePoints = examAttempts
    .filter(
      (a): a is typeof a & { submittedAt: Date; percentage: number } =>
        a.submittedAt !== null && a.percentage !== null,
    )
      ...
      <BinderPageShell user={user} homeHref="/mentor">
        <TabbedCard>
          <div className="mx-auto max-w-3xl">
            ...
            {!student.isActive && (
              <p className="mt-1 text-sm text-danger">Deactivated</p>
            )}
                  ...
                  >
                    <span className="text-foreground-muted">
                      ...
                      <span className="text-foreground-subtle">
                        ({e.event.cluster.name})
                      </span>
                    </span>
                    <span className="text-foreground-subtle">
                      {e.isCurrent
                        ? "Current"
                        : `Ended ${e.endedAt?.toLocaleDateString() ?? ""}`}
                    </span>
                  </li>
                ))}
              ...
              <Card>
                ...
                <ul className="space-y-1 text-sm">
                  ...
                  {examAttempts.length === 0 && (
                    <li className="text-foreground-muted">No attempts yet.</li>
                  )}
                </ul>
              </Card>
              ...
              <Card>
                ...
                <ul className="space-y-1 text-sm">
                  ...
                  {areaBreakdown.length === 0 && (
                    <li className="text-foreground-muted">No exam data yet.</li>
                  )}
                </ul>
              </Card>
            </div>
                ...
                <ul className="space-y-1 text-sm">
                  {masteryEstimates.map((a) => (
                    ...
                    >
                      ...
                      <span className="text-foreground-subtle">
                        {Math.round(a.estimatedMastery * 100)}%
                      </span>
                    </li>
                  ))}
                </ul>
            ...
            <Card className="mt-4">
              ...
              <ul className="space-y-1 text-sm">
                ...
                {submissions.length === 0 && (
                  <li className="text-foreground-muted">No submissions yet.</li>
                )}
              </ul>
            </Card>
            ...
            <Card className="mt-4">
              ...
              <ul className="space-y-1 text-sm">
                ...
                {activityLog.length === 0 && (
                  <li className="text-foreground-muted">No activity yet.</li>
                )}
              </ul>
            </Card>
            ...
            <Card className="mt-4">
              <p className="mb-2 font-medium text-foreground">
                Mentor notes &amp; interventions
              </p>
              <ul className="mb-4 space-y-2 text-sm">
                {interventions.map((i) => {
                  const impact = computeInterventionImpact(
                    i.createdAt,
                    percentagePoints,
                  );
                  return (
                    ...
                    >
                      ...
                      <p className="text-foreground-subtle">
                        ...
                        {i.instructionalArea
                          ? ` · ${i.instructionalArea.name}`
                          : ""}
                      </p>
                      {impact.change !== null ? (
                        <p
                          className={
                            impact.change >= 0 ? "text-success" : "text-danger"
                          }
                        >
                          {Math.round(impact.beforeAvg!)}% →{" "}
                          {Math.round(impact.afterAvg!)}% (
                          {impact.change >= 0 ? "+" : ""}
                ...
                {interventions.length === 0 && (
                  <li className="text-foreground-muted">No notes yet.</li>
                )}
              </ul>
              <InterventionForm
