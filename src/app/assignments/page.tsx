...
function statusBadge(assignment: {
  ...
  if (assignment.type === "EXAM") {
    ...
    if (!attempt) return { label: "Not started", className: "bg-surface-hover text-foreground-muted" };
  ...
  if (!submission) return { label: "Not started", className: "bg-surface-hover text-foreground-muted" };
  switch (submission.status) {
    case "GRADED":
      return { label: "Graded", className: "bg-success/20 text-success" };
    case "RESUBMISSION_REQUESTED":
      return { label: "Resubmission requested", className: "bg-danger/20 text-danger" };
    case "LATE":
      return { label: "Submitted late", className: "bg-accent-soft text-accent" };
    default:
