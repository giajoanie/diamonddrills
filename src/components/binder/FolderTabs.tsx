...
export function FolderTabs({ tabs }: { tabs: FolderTab[] }) {
  return (
    <div className="flex flex-wrap gap-1.5 px-2">
      {tabs.map((tab) => {
        const className = `rounded-t-xl border-t-2 px-4 py-2.5 text-sm font-bold font-display transition-colors sm:px-5 ${
          tab.active
            ? "border-highlight bg-background-elevated text-accent-strong"
            : "border-transparent bg-surface-hover text-foreground-muted hover:bg-border/60 hover:text-foreground"
        }`;
